...
class ParamSimple() {
  ...
  var fpuWbAt = 2
  var fpuWbWriteAt = 9
  var fpuBypassFrom = -1 // -1 => allowBypassFrom.max(2)
  ...
  var fetchL1HitsAt = 1
  var fetchL1HitAt = 1
  var fetchL1BankMuxesAt = 1
  var fetchL1BankMuxAt = 2
  var fetchL1CtrlAt = 2
  ...
  var lsuL1BankReadAt = 0
  var lsuL1WayReadAt = 0
  var lsuL1HitsAt = 1
  var lsuL1HitAt = 2
  var lsuL1BankMuxesAt = 1
  var lsuL1BankMuxAt = 2
  var lsuL1CtrlAt = 2
  var gshareReadAt = 0
  var historyFetchBypass = true
  ...
  def fetchL1PmpTiming = new PmpPortParameter(
    napotMatchAt = fetchL1ReadAt + 1,
    napotHitsAt = fetchL1ReadAt + 1,
    torCmpAt = fetchL1ReadAt + 1,
    torHitsAt = fetchL1ReadAt + 2,
    hitsAt = fetchL1ReadAt + 2,
    rspAt = fetchL1ReadAt + 2
  )
  ...
  def alignerPluginFetchAt = fetchL1Enable.mux(fetchL1CtrlAt, 1+fetchForkAt)
...
//    lsuForkAt = 1
  ...
  def addOptions(parser: scopt.OptionParser[Unit]) = {
    ...
    opt[Int]("fpu-wb-at") action { (v, c) => fpuWbAt = v }
    opt[Int]("fpu-wb-write-at") action { (v, c) => fpuWbWriteAt = v }
    opt[Int]("fpu-bypass-from") action { (v, c) => fpuBypassFrom = v }
    opt[Int]("fpu-add-preshift-stage") action { (v, c) => fpuAddSharedParam.preShiftStage = v }
    opt[Int]("fpu-add-shifter-stage") action { (v, c) => fpuAddSharedParam.shifterStage = v }
    opt[Int]("fpu-add-math-stage") action { (v, c) => fpuAddSharedParam.mathStage = v }
    opt[Int]("fpu-add-norm-stage") action { (v, c) => fpuAddSharedParam.normStage = v }
    opt[Int]("fpu-add-pack-at") action { (v, c) => fpuAddSharedParam.packAt = v }
    ...
    opt[Int]("fetch-l1-read-at") action { (v, c) => fetchL1ReadAt = v }
    opt[Int]("fetch-l1-hits-at") action { (v, c) => fetchL1HitsAt = v }
    opt[Int]("fetch-l1-hit-at") action { (v, c) => fetchL1HitAt = v }
    opt[Int]("fetch-l1-bank-muxes-at") action { (v, c) => fetchL1BankMuxesAt = v }
    opt[Int]("fetch-l1-bank-mux-at") action { (v, c) => fetchL1BankMuxAt = v }
    opt[Int]("fetch-l1-ctrl-at") action { (v, c) => fetchL1CtrlAt = v }
    ...
    opt[Int]("lsu-l1-bank-read-at") action { (v, c) => lsuL1BankReadAt = v }
    opt[Int]("lsu-l1-way-read-at") action { (v, c) => lsuL1WayReadAt = v }
    opt[Int]("lsu-l1-hits-at") action { (v, c) => lsuL1HitsAt = v }
    opt[Int]("lsu-l1-hit-at") action { (v, c) => lsuL1HitAt = v }
    opt[Int]("lsu-l1-bank-muxes-at") action { (v, c) => lsuL1BankMuxesAt = v }
    opt[Int]("lsu-l1-bank-mux-at") action { (v, c) => lsuL1BankMuxAt = v }
    opt[Int]("lsu-l1-ctrl-at") action { (v, c) => lsuL1CtrlAt = v }
    ...
    opt[Int]("gshare-read-at") action { (v, c) => gshareReadAt = v }
    opt[Unit]("no-history-fetch-bypass") action { (v, c) => historyFetchBypass = false }
  ...
  def pluginsArea(hartId : Int = 0) = new Area {
    ...
    if(withBtb) {
      plugins += new prediction.BtbPlugin(
        ...
        rasDepth = if(withRas) 8 else 0,  // openfpgaOS: Quake's recursive BSP walk blows a 4-deep RAS; 8 covers typical depth 10-25 call chains far better for ~4 extra MLAB entries
    ...
    if(withGShare) {
      ...
      plugins += new prediction.HistoryPlugin(historyFetchBypass = historyFetchBypass)
    }
    ...
    if(fetchL1Enable) {
      plugins += new fetch.FetchL1Plugin(
        ...
        readAt = fetchL1ReadAt,
        hitsAt = fetchL1HitsAt,
        hitAt = fetchL1HitAt,
        bankMuxesAt = fetchL1BankMuxesAt,
        bankMuxAt = fetchL1BankMuxAt,
        ctrlAt = fetchL1CtrlAt,
        ...
        pmpPortParameter = fetchL1PmpTiming
      )
    ...
    if(lsuL1Enable){
      ...
      plugins += new LsuL1Plugin(
        ...
        setCount       = lsuL1Sets,
        wayCount       = lsuL1Ways,
        bankReadAt     = lsuL1BankReadAt,
        wayReadAt      = lsuL1WayReadAt,
        hitsAt         = lsuL1HitsAt,
        hitAt          = lsuL1HitAt,
        bankMuxesAt    = lsuL1BankMuxesAt,
        bankMuxAt      = lsuL1BankMuxAt,
        ctrlAt         = lsuL1CtrlAt,
...
//      }
    ...
    if (withRvf || withRvd) {
      ...
      plugins += new WriteBackPlugin(lane0, FloatRegFile, writeAt = fpuWbWriteAt, allowBypassFrom = if(fpuBypassFrom >= 0) fpuBypassFrom else allowBypassFrom.max(2)) //Max 2 to save area on not so important instructions
