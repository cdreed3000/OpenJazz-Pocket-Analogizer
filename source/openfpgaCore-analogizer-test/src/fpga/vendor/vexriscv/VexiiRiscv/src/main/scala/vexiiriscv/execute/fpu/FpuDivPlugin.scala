...
class FpuDivPlugin(val layer : LaneLayer,
  ...
  val logic = during setup new Area{
    ...
    val onExecute = new layer.Execute(exeAt) {
      // Registered zero-numerator flag, same pattern as FpuSqrtPlugin.
      // Keeping the raw RS1_FP.isZero compare in the inject gate puts a
      // combinational exponent/mantissa==0 cone from the execute-stage
      // float RS1 payload registers straight onto DivPlugin's
      // processing.request -> execute_freeze_valid — the worst setup
      // chain on Cyclone V at 100 MHz.  With the register, a
      // zero-numerator FDIV freezes one extra cycle then releases via
      // the forceZero pack override below; the relaxed-div relaxer
      // masks the divider cmd on that cycle so no spurious op starts.
      val isZero = RegInit(False) setWhen (fup.unpackingDone(exeAt) && RS1_FP.isZero) clearWhen (isReady)
      when(isValid && SEL && fup.unpackingDone(exeAt) && !isZero) {
