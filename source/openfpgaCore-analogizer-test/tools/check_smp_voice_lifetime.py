...
def main():
    ...
    parser.add_argument("--source", type=Path,
                        default=ROOT / "src/firmware/api/of_smp_voice.c")
    ...
    with tempfile.TemporaryDirectory(prefix="openfpgaos-voices-") as tmp:
        build = Path(tmp)
        (build / "include").symlink_to(ROOT / "src/firmware/api", target_is_directory=True)
        ...
        subprocess.run([
            ...
            str(ROOT / "src/firmware/api/of_smp_tables.c"), "-o", str(binary),
        ], check=True)
