...
SetupOptions Setup::load () {
	...
	File* file;
	SetupOptions cfg = { false, 0, 0, false, 0, scalerType::None, 1 };
	...
	// Config v8 adds one byte for the OpenFPGA presentation mode.
	// Keep accepting v7 so an existing user's config migrates automatically.
	int configVersion = file->loadChar();
	if ((configVersion != 7) && (configVersion != 8)) {
	...
	cfg.presentationMode = (configVersion >= 8)? CLAMP(file->loadChar(), 0, 2): 1;
	setup.presentationMode = cfg.presentationMode;
...
void Setup::save () {
	...
	file->storeChar(CLAMP(setup.presentationMode, 0, 2));
