...
int SetupMenu::setupKeyboard () {
	...
	while (true) {
		...
		for (int count = 0; count < PCONTROLS; count++) {
			...
			if (count < progress)
				fontmn2->showString(controls.getKeyName(count),
					(canvasW >> 1) + 16, (canvasH >> 1) + (count << 4) - 56);
...
#if defined(OPENFPGA)
	const char* setupPresentationOptions[3] = {
		"DOS 4:3 FILL",
		"320x200 CENTER",
		"POCKET 320x288"
	};
...
#if defined(OPENFPGA)
				suboption = CLAMP(setup.presentationMode, 0, 2);

				while (true) {
					ret = generic("VIDEO PRESENTATION",
						setupPresentationOptions, 3, suboption);

					if (ret == E_QUIT) return E_QUIT;
					if (ret < 0) break;

					int previousMode = setup.presentationMode;
					setup.presentationMode = suboption;
					of_sdl_SetPresentationMode(suboption);

					/* NO_RESIZE ignores the numeric arguments; reset rebuilds
					 * the logical canvas + physical window from the mode above. */
					if (!video.reset(video.getWidth(), video.getHeight())) {
						setup.presentationMode = previousMode;
						of_sdl_SetPresentationMode(previousMode);
						video.reset(video.getWidth(), video.getHeight());
						if (message("VIDEO MODE FAILED") == E_QUIT) return E_QUIT;
					} else {
						playConfirmSound();
					}

					/* reset() recreates the indexed video surfaces. */
					video.setPalette(menuPalette);
				}
#elif defined(NO_RESIZE) && !defined(SCALE)
				if (message("FEATURE NOT AVAILABLE") == E_QUIT) return E_QUIT;
#else
