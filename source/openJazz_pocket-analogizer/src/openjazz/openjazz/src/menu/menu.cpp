...
#if defined(OPENFPGA)
		/* OPENFPGA Pocket: B is the normal menu-back button.
		 *
		 * APF button IDs expose Pocket B as joystick button 1. OpenJazz's
		 * stock defaults map button 1 to C_JUMP, while C_ESCAPE defaults to
		 * joystick button 5 (which the Pocket does not expose as a face
		 * button). Treat C_JUMP as Back ONLY inside generic menus. This keeps
		 * B's normal gameplay action unchanged because gameplay code is not
		 * affected by this menu-only check. */
		if (controls.release(C_ESCAPE) || controls.release(C_JUMP))
			return E_RETURN;
#else
...
#if defined(OPENFPGA)
		/* Match the actual Pocket controller behavior instead of advertising
		 * a keyboard key that is not mapped to the handheld controls. */
		fontbig->showString("(B) BACK", canvasW - 3, canvasH,
