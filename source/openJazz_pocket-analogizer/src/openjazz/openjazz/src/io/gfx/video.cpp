...
#if defined(OF_ANALOGIZER_CRT) && defined(OPENFPGA)
	of_sdl_SetPresentationMode(cfg.presentationMode);
#endif
...
bool Video::reset (int width, int height) {
	...
	if (of_sdl_AnalogizerVideoEnabled()) {
		switch (of_sdl_GetPresentationMode()) {
			case OF_PRESENTATION_POCKET_288:
				/* OpenJazz Pocket's expanded viewport. */
			...
			default:
				/* Original Jazz logical game canvas. CENTER_200 differs only
				 * in the physical window size selected by of_sdl_SetWindowSize. */
				screenW = SW;
				screenH = SH;
				break;
		}
	...
	// The Pocket NO_RESIZE limits are normally fixed at 320x288.  Our
	// Analogizer modes intentionally use a 320x200 logical canvas, so do not
	// "sanitize" that back to 288 before the platform wrapper can present it.
	bool customPresentation = false;
#if defined(OF_ANALOGIZER_CRT) && defined(OPENFPGA)
	customPresentation = of_sdl_AnalogizerVideoEnabled();
#endif
	if (!customPresentation &&
	    (screenW < minW || screenW > maxW || screenH < minH || screenH > maxH)) {
